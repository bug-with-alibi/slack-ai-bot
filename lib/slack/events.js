...
function resolveInstallationContext(body) {
  return {
    appId: body.api_app_id || body.authorizations?.[0]?.app_id || null,
    teamId: body.team_id || body.authorizations?.[0]?.team_id || null,
    enterpriseId:
      body.enterprise_id || body.authorizations?.[0]?.enterprise_id || null
  };
}
...
export function createSlackEventsHandler({
  ...
  return async function handleSlackEvents(req, res) {
    ...
    const { appId, teamId, enterpriseId } = resolveInstallationContext(req.body);

    if (!appId || (!teamId && !enterpriseId)) {
      logger.warn("[events] unable to resolve installation context for Slack event");
      return;
    }
    ...
    try {
      logger.log(
        `[events] looking up installation for app=${appId} teamId=${teamId || "n/a"} enterpriseId=${enterpriseId || "n/a"}`
      );
      installation = await installationStore.getBySlackContext({
        appId,
        teamId,
        enterpriseId
      });
      ...
      if (!installation?.botToken) {
        logger.warn(
          `[events] no installation found for app=${appId} teamId=${teamId || "n/a"} enterpriseId=${enterpriseId || "n/a"}`
        );
        return;
      }
      ...
      logger.log(
        `[events] posting placeholder reply - workspace=${installation.teamName || installation.enterpriseName || teamId || enterpriseId} channel=${event.channel}`
      );
      ...
      logger.log(
        `[events] generating LLM response - provider=${llmProvider} workspace=${installation.teamName || installation.enterpriseName || teamId || enterpriseId} contextMessages=${memoryContext.selectedMessages.length}`
      );
